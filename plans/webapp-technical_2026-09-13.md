...
- 2026-09-13 — **Step 10 DONE (quick-add, mobile-first) + admin HOSTED.** Owner decisions: channel = web app (iPhone + rarely-Telegram killed the bot option; WhatsApp rejected — Meta business platform overkill), hosting = GitHub Pages on the existing repo (`webapp` branch now pushed public — owner chose this knowingly). Architecture: (a) `resolve-place` **Edge Function** (`app/supabase/functions/`) replaces the dev-server proxy entirely — expands `maps.app.goo.gl` short links server-side (browsers can't follow cross-origin redirects), extracts FTID/CID + name + coords from the expanded URL, then searchText with coord-circle or NYC-rect bias; deployed `--no-verify-jwt` (new-style keys aren't JWTs) with in-function owner check via `auth.getUser()` — verified live: non-owner gets 401. Key in a function secret. (b) Admin deployed to https://k-mizrahi.github.io/nyc-food/admin/ via `app/scripts/deploy_admin.sh` — builds with base `/nyc-food/admin/`, commits only the `admin/` dir onto origin/main through a throwaway worktree (index.html byte-untouched, verified post-deploy; "never merge webapp into main" holds). (c) Quick Add tab: paste link or name → resolve → dedupe check against `places.cid`, `places.google_place_id`, and pending `import_rows` (warns instead of double-adding) → prefilled triage-style form → insert with tags. (d) Login gains 6-digit OTP entry (`verifyOtp`) — iOS magic-link taps land sessions in the wrong browser context. (e) `SUPABASE_ACCESS_TOKEN` added to `.env` for CLI deploys (`app/scripts/deploy_functions.sh`; near-loss of `.env` during manual edit — recovered via TextEdit undo). Owner still to do in dashboard: add the Pages URL to Auth redirect URLs; add `{{ .Token }}` to the Magic Link email template so the code appears.
- 2026-09-13 — **Step 7 (interactive half) DONE — "Auto-fill from Google", and the open architecture question is RESOLVED: no Edge Function for Phase 1.** A dev-only Vite middleware (`/api/resolve-place` in `vite.config.ts`) holds `GOOGLE_MAPS_API_KEY` (via `loadEnv` from repo-root `.env`) and makes the outbound call — key never reaches the browser; prod builds have no endpoint at all. One `places:searchText` per click, NYC-rectangle locationBias, field mask `places.id,displayName,formattedAddress,location,addressComponents` (Essentials+Pro; Enterprise ban holds). Borough derived from `sublocality_level_1` ('The Bronx'→'Bronx'; fallback locality 'New York'+NY → Manhattan); neighborhood from the `neighborhood` component when present (verified live: Katz's returns Manhattan but NO neighborhood — Google's NYC neighborhood coverage is patchy as predicted, owner types the rest). On keep/save the match also writes `google_place_id`, `address`, `lat`, `lng`, `refreshed_at`. Google's name+address surface next to the form for bad-match catching. The monthly refresh job (step 9) still goes through GH Actions/Python — the Edge Function decision applied only to interactive calls.
- 2026-09-13 — **Tag curation live (step 8, owner request: multi-select "type" labels)**. TagPicker in both keep and edit forms: chips + suggestions from `tags`, typing an unknown label creates the tag on the fly (`label_en` = typed text; slug keeps any-script letters so Hebrew labels work; slug collision returns the existing tag). Writes `place_tags` with `source='owner'`; edit-save syncs (inserts missing, deletes removed); keep-merge unions tags additively. `label_he` stays null for now — fill during Phase 2 translation pass.
- 2026-09-13 — **Keep now merges on existing CID** (owner hit `places_cid_key` violation live). `keepGroup` first looks up `places` by CID: on match it links lists (`place_lists` upsert, ignore dupes) and marks rows kept against the existing place, leaving its fields untouched — the create/merge semantics step 6 always specified. Also closed a double-fire race (two rapid `k` presses both passed the stale React `busy` state; a synchronous ref guards now) and made place-added state dedupe by id.
- 2026-09-13 — **Places tab added** (owner request during triage: edit kept places). Searchable list of all created places (non-NYC badged), edit panel with the full keep-form field set incl. `in_nyc`, count-verified save, and "send back to queue" (deletes the place — `place_lists` cascades — and resets its import rows to pending; refuses if no rows point at the place). `fetchAll`/`keepGroup` now select the full place column set (still `cid::text`). `published` deliberately not editable yet — nothing public exists to publish to; revisit in Phase 2.
- 2026-09-13 — **Owner decision during triage: `places.in_nyc` flag** (migration `0002_add_in_nyc.sql`). Non-NYC places from the lists are kept, not dropped — flagged for possible future city expansion. Boolean, default true, admin-only: deliberately NOT in `public_places` (`published` stays the only public gate; a city facet supersedes this flag if expansion ever happens). Keep form has the checkbox. Applied by owner via SQL Editor.
- 2026-09-13 — **Steps 5–6 DONE** (login + owner RLS verified live by owner first — saw 42/1,015/1,015). Admin is now tabbed: overview / lists / review. Decisions: (a) **CID is fetched as text everywhere** (`select=cid::text`, verified accepted by live PostgREST) — 64-bit CIDs exceed JS safe-integer range and would silently corrupt as JSON numbers; it stays a string end to end, inserts cast back to numeric server-side. (b) **No bulk keep on the Lists screen** — keep must create a `places` row via the form, so keeps go through the review queue only; Lists offers bulk *drop pending* + *restore dropped* (restore skips rows with a `place_id`), `default_action` toggle persists as a marker. (c) Review queue groups pending rows by CID (no-CID rows are singleton groups), list filter, keyboard: ↑/↓/j move, k keep, d drop, u undo, e edit, esc back to shortcuts. (d) Keep form prefills: notes split EN/HE by Hebrew-char detection, rec_source suggested from "המלצה של/המלצת X" pattern (suggestion only), status defaults want_to_try; cuisine/borough datalists (cuisine builds from already-created places). (e) Keep writes `places` (slug with collision-retry suffix; Hebrew-only names fall back to `place-<cid>`), `place_lists` for every source list, rows → kept+place_id; `gmaps_url` canonicalized to `https://maps.google.com/?cid=<cid>`. (f) Single-level undo: keep-undo deletes the place (place_lists cascades) and restores rows; drop-undo restores. (g) All mutations verify affected-row counts and throw on mismatch — no silent partial writes. Build clean.
